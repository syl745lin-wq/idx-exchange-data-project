...
WEEK23_DIR = OUTPUT_DIR / "week2_3" / "listed"
MORTGAGE_DIR = OUTPUT_DIR / "week2_3" / "mortgage_enrichment"
WEEK45_DIR = OUTPUT_DIR / "week4_5" / "listed"
WEEK6_DIR = OUTPUT_DIR / "week6" / "listed"
...
WEEK6_REQUIRED_FIELDS = ["ListingId", "ListingContractDate", "ListPrice"]
...
def filter_residential(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    print("\n===== LISTED RESIDENTIAL FILTER CHECK =====")
...
def save_numeric_plots(df: pd.DataFrame, plot_dir: Path) -> None:
    ...
    for field in [f for f in NUMERIC_FIELDS if f in df.columns]:
        ...
        ax.set_title(f"listed histogram: {field}")
        ax.set_xlabel(field)
        ax.set_ylabel("count")
        fig.tight_layout()
        fig.savefig(plot_dir / f"listed_{field}_hist.png", dpi=150)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(8, 2.8))
        ax.boxplot(series, vert=False)
        ax.set_title(f"listed boxplot: {field}")
        ax.set_xlabel(field)
        fig.tight_layout()
        fig.savefig(plot_dir / f"listed_{field}_box.png", dpi=150)
        plt.close(fig)
...
def run_week23_analysis() -> Dict[str, Path]:
    ...
    combined_df, row_count_df = load_and_concat("CRMLSListing*.csv")
    ...
    paths = {
        "combined": WEEK23_DIR / "listed_combined_week23.csv",
        "filtered": WEEK23_DIR / "listed_residential_week23.csv",
        "dtypes": WEEK23_DIR / "listed_dtypes.csv",
        "missing": WEEK23_DIR / "listed_missing_value_report.csv",
        "high_missing": WEEK23_DIR / "listed_high_missing_gt90.csv",
        "retention": WEEK23_DIR / "listed_column_retention_report.csv",
        "numeric": WEEK23_DIR / "listed_numeric_distribution_summary.csv",
        "outlier": WEEK23_DIR / "listed_outlier_summary.csv",
        "property_before": WEEK23_DIR / "listed_property_type_before_filter.csv",
        "property_after": WEEK23_DIR / "listed_property_type_after_filter.csv",
        "filter_validation": WEEK23_DIR / "listed_filter_validation.csv",
        "questions": WEEK23_DIR / "listed_intern_questions.csv",
        "row_counts": WEEK23_DIR / "listed_row_counts.csv",
    }
...
def run_mortgage_enrichment(filtered_input: Path) -> Path:
    ...
    listed = pd.read_csv(filtered_input, low_memory=False)
    ...
    listed["year_month"] = listed["ListingContractDate"].dt.to_period("M").astype(str)
    listed_with_rates = listed.merge(monthly_rates, on="year_month", how="left")
    validation = pd.DataFrame(
        [
            {"metric": "input_rows", "value": len(listed)},
            {"metric": "output_rows", "value": len(listed_with_rates)},
            {"metric": "null_rate_values", "value": int(listed_with_rates["mortgage30us_monthly_avg"].isna().sum())},
        ]
    )
    ...
    listed_with_rates.to_csv(MORTGAGE_DIR / "listed_residential_with_mortgage_rates.csv", index=False)
    validation.to_csv(MORTGAGE_DIR / "listed_mortgage_merge_validation.csv", index=False)
    return MORTGAGE_DIR / "listed_residential_with_mortgage_rates.csv"
...
def remove_critical_invalid_rows(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["ListingId", "ListingContractDate", "ListPrice"]
...
def run_week6_analysis(input_path: Path) -> Dict[str, Path]:
    ...
    paths = {
        ...
        "outlier_summary": WEEK6_DIR / "listed_outlier_summary_week6.csv",
...
def build_week6_missing_action_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in df.columns:
        ...
        elif column in {"CloseDate", "ClosePrice", "PurchaseContractDate"}:
            action = "retained nulls when business-valid for active or not-yet-closed listings"
...
def week6_transformation_rows() -> List[Dict[str, object]]:
    return [
        {"step": 1, "transformation": "load_week45_cleaned_output", "reason": "Build Week 6 preparation on top of the cleaned Week 5 listed dataset."},
        {"step": 2, "transformation": "reconfirm_datetime_columns", "reason": "Ensure listing timeline fields are consistently typed as datetime."},
        ...
        {"step": 6, "transformation": "document_missing_value_handling", "reason": "Differentiate business-valid missing closing fields from problematic missing core listing fields."},
    ]
...
def run_week6_preparation(input_path: Path) -> Dict[str, Path]:
    ...
    paths = {
        "prepared": WEEK6_DIR / "listed_analysis_ready_week6.csv",
        "date_check": WEEK6_DIR / "listed_week6_date_conversion_check.csv",
        "numeric_check": WEEK6_DIR / "listed_week6_numeric_conversion_check.csv",
        "missing_summary": WEEK6_DIR / "listed_week6_missing_summary.csv",
        "missing_action_summary": WEEK6_DIR / "listed_week6_missing_action_summary.csv",
        "column_inventory": WEEK6_DIR / "listed_week6_column_inventory.csv",
        "transformation_log": WEEK6_DIR / "listed_week6_transformation_log.csv",
    }
...
def run_week45_cleaning(input_path: Path) -> Dict[str, Path]:
    ...
    paths = {
        "cleaned": WEEK45_DIR / "listed_analysis_ready_week45.csv",
        "row_counts": WEEK45_DIR / "listed_week45_row_counts.csv",
        "transformation_log": WEEK45_DIR / "listed_transformation_log.csv",
        "dtypes": WEEK45_DIR / "listed_week45_dtypes.csv",
        "date_flags": WEEK45_DIR / "listed_date_flag_summary.csv",
        "geo_summary": WEEK45_DIR / "listed_geographic_quality_summary.csv",
        "missing_summary": WEEK45_DIR / "listed_missing_summary.csv",
        "invalid_numeric_summary": WEEK45_DIR / "listed_invalid_numeric_summary.csv",
    }
